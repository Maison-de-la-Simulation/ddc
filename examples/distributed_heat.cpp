...
using DVecX = DiscreteVector<DDimX>;
using DDomX = DiscreteDomain<DDimX>;

//! [Y-space]
// Uniform discretization of our second continuous dimension
using DDimY = UniformDiscretization<struct Y>;
//! [Y-space]

using DVecY = DiscreteVector<DDimY>;
using DDomY = DiscreteDomain<DDimY>;

//! [time-space]
// Uniform discretization of our simulated time dimension
using DDimT = UniformDiscretization<struct T>;
//! [time-space]

using DVecT = DiscreteVector<DDimT>;
using DDomT = DiscreteDomain<DDimT>;
...
int main(int argc, char* argv[])
{
    ...
    auto const& xy_distrib = CartesianMpiDistribution<
            DDimX,
            DDimY>(MPI_COMM_WORLD, DiscreteVector<DDimX, DDimY>(3, 3));

    auto const& xy_ghosted_distrib = xy_distrib.with_overlap({gwx, gwy});
